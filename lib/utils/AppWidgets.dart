...
Widget itemTitle(BuildContext context, String titleText,
    {double fontSize = ts_normal, int? maxLine, TextAlign? textAlign}) {
  return Text(
    titleText,
    style: TextStyle(
      fontSize: fontSize,
...
Widget itemSubTitle(BuildContext context, String titleText,
    {double fontSize = ts_normal, Color? textColor, isLongText = true}) {
  return Text(
    titleText,
    style: TextStyle(
      fontSize: fontSize,
...
Widget headingText(var titleText, {int? fontSize, int maxLines = 1}) {
  return Text(titleText,
      style: primaryTextStyle(size: fontSize ?? 18),
      maxLines: maxLines,
      overflow: TextOverflow.ellipsis);
}

Widget headingWidViewAll(BuildContext context, var titleText,
    {VoidCallback? callback, bool showViewMore = true}) {
...
Widget button(BuildContext context, buttonText,
...
Widget subType(context, VoidCallback callback, icon,
    {bool showTrailIcon = true, String? title}) {
...
Widget hdWidget(context) {
  return Container(
    ...
    child:
        Text("HD", style: primaryTextStyle(color: textColorPrimary, size: 10)),
  );
}
...
InputDecoration inputDecoration({String? hint, IconData? suffixIcon}) {
  return InputDecoration(
    ...
    enabledBorder:
        OutlineInputBorder(borderSide: BorderSide(color: Colors.grey.shade500)),
    disabledBorder:
        OutlineInputBorder(borderSide: BorderSide(color: Colors.grey.shade500)),
    labelText: hint,
...
Widget commonCacheImageWidget(String? url,
  ...
  if (url.validate().startsWith('http')) {
    if (isMobile) {
      ...
      return Image.network(url.validate(),
          height: height, width: width, fit: fit, color: color);
    }
  } else {
    return Image.asset(url.validate(),
        height: height, width: width, fit: fit, color: color);
  }
}
...
List<LanguageDataModel> languageList() {
  return [
    ...
    LanguageDataModel(
        ...
        flag: 'assets/images/flag/ic_india.png'),
    LanguageDataModel(
        id: 3,
        name: 'Arabic',
        languageCode: 'ar',
    ...
    LanguageDataModel(
        ...
        flag: 'assets/images/flag/ic_france.png'),
  ];
}

String userProfileImage() {
  Random random = Random();
  return 'assets/icons/ic_icon_tm.png';
